...
func _on_Area_body_entered(body):
	if body.name == "Player" and visible == true:
			$Chime.play()
			GameManager.greenEmerladsCollected+=1
			colleted = GameManager.greenEmerladsCollected
			GameManager.dead=true
			dead=true
			Save.green_position(global_transform.origin.x,global_transform.origin.y,global_transform.origin.z)
...
func Load():
	...
	if Save.loadpos == 1 and file.file_exists(Save.path_one):
		...
		for i in result2["green_x"]:
			for k in result2["green_y"]:
				for z in result2["green_z"]:
